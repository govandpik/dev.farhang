  ...
  **34: ** [\`${Radiostations[34-1].split(" ")[0]}\`](${Radiostations[34-1].split(" ")[1]})`, inline: true },
    ...
    if(!message.guild)      
      ...
      if (args[0] == null) {
        ...
        .setDescription(`** <a:emoji_26:809385634149826611> ${message.author} Check your \`direct messages\` for a list of Radio Stations!**`)
        );
        message.author.send(new MessageEmbed().setColor("BLUE")
        .setDescription(`** <a:emoji_26:809385634149826611> Sent from <#${message.channel.id}>**`))
        return message.author.send(resultsEmbed);
      }
  ...
  message.channel.send(    new MessageEmbed().setColor("BLUE")
  .setDescription(`** <a:emoji_26:809385634149826611> Joined \`${channel.name}\` 📄 bouned \`#${message.channel.name}\`**`)
  .setFooter(`${message.author.username}#${message.author.discriminator}`));
  //send Search something embed
  message.channel.send(new MessageEmbed().setColor("BLUE")
  .setDescription(`** <:emoji_38:813401541510692946> Searching 🔍 \`${Radiostations[i-1].split(" ")[0]}\`**`));
  //mute yourself
